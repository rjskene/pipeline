---
name: pipeline
description: Check the status of the GitHub issue pipeline and advance the next stage. Run this at the start of any coding session to see what is ready and propose next actions.
...
1. **Plan** — before dispatching plan-issue, run `/classify-issue N` for every ready issue that lacks a fresh Classification comment (dispatch in parallel, one Agent per issue). Each classify run writes the Classification comment AND applies the path label (`docs-only` or `multi-task`). Cached issues skip dispatch. Then run `/plan-issue N` for every issue with no pipeline label (in parallel, one Agent per issue). Wait for all to complete.
   - **Verify plan comments:** After all plan-issue agents complete, for each issue that was targeted (had no pipeline label at the start of this step), confirm a plan comment was posted:
     ...
     ```
     If any targeted issue has `PLAN_COUNT == 0` (regardless of whether `plan-pending` was added), the plan-issue agent failed. Re-run `/plan-issue N` for that issue (max 1 retry). If still missing after retry, skip the issue and flag it in the final report as "Skipped (plan not posted)".
2. **Evaluate plans** — run `/evaluate-issue-plan N` for every `plan-pending` issue (in parallel, one Agent per issue). Wait for all to complete.
3. **Re-plan loop** — for any issue whose evaluation verdict is "Revise": re-run `/plan-issue N`, then `/evaluate-issue-plan N`. Repeat until all pass (max 3 iterations per issue). If an issue still fails after 3 iterations, skip it and flag it in the final report.
...
7. **Evaluate PRs** — once all agents finish (queue complete), run `/evaluate-issue-pr N` for every `pr-open` issue (via `run-queue.sh --skip-permissions --skill evaluate-issue-pr`). Launch this queue via `Bash` with `run_in_background: true` as described in step 6.
...
- Worktree path: `.claude/worktrees/wt-<issue_number>-<slug>` (e.g., `.claude/worktrees/wt-25-gantt-contract-creation`)
...
0. **Housekeeping — branch check + subtree drift check + sync worktrees + kill stale sessions**

   **First, confirm the orchestrator is on the configured base branch.** The base branch for all PRs is read from `pipeline.config` (`PIPELINE_BASE_BRANCH=staging`). The orchestrator session should be on that branch so spawned worktrees inherit from it and PRs target it.

   ```bash
   EXPECTED_BASE="staging"
   ...
   ```bash
   NEXT_ISSUES=$(gh issue list --repo HTS-COLLAB-ORG/claude-pipeline --state open \
   ...
   Then, check if `.claude-pipeline/` is in sync with the source repo and resolve any drift:
   ```bash
   bash .claude/scripts/resolve-subtree-drift.sh 2>/dev/null; DRIFT_EXIT=$?
   ```
   - **Interactive mode** (default): the script prompts for each action (pull? push? reinstall?). Follow the prompts.
   - **Full-send mode**: pass `--auto` — auto-pulls + reinstalls without prompting. Never auto-pushes (push is a sharing decision).
     ```bash
     bash .claude/scripts/resolve-subtree-drift.sh --auto 2>/dev/null; DRIFT_EXIT=$?
     ```
   - If the script reports "Skills updated", note it in the housekeeping output but do not stop — spawned agents read updated skills from disk.
   ...
   Then check for stale tmux sessions from previous pipeline runs. If a tmux `dev` session exists, kill any leftover queue runner and agent windows:
   ```bash
   # List all windows in the dev session
   tmux list-windows -t dev -F '#{window_name}' 2>/dev/null
   ```
   ...
   for win in $(tmux list-windows -t dev -F '#{window_name}' 2>/dev/null | grep '^issue-'); do
     tmux kill-window -t "dev:${win}" 2>/dev/null || true
   done
   # Interrupt any running queue runner in the first window
   tmux send-keys -t dev:0 C-c 2>/dev/null || true
   ```
...
2. **Discover pipeline issues** — fetch all open AND recently closed issues, and classify by label:
   ```bash
   gh issue list --repo HTS-COLLAB-ORG/claude-pipeline --state open --json number,title,labels --limit 100
   gh issue list --repo HTS-COLLAB-ORG/claude-pipeline --state closed --json number,title,labels --limit 20
...
5. **Propose ONE action** based on state priority:
   ...
   - Else if any issues have `pr-open`:
     - For each, check if the PR has an evaluation comment:
       ...
       gh pr view $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --json comments --jq '[.comments[] | select(.body | contains("## Evaluation"))] | length'
       ```
     - If any `pr-open` issues have NO evaluation comment → propose running `/evaluate-issue-pr` for those issues (in their existing worktrees).
   ...
   - Else if any issues have `plan-pending`:
     - For each, check if the issue has a plan evaluation comment:
       ```bash
       gh issue view $N --repo HTS-COLLAB-ORG/claude-pipeline --json comments --jq '[.comments[] | select(.body | contains("## Plan Evaluation"))] | length'
       ```
     - If any `plan-pending` issues have NO plan evaluation → propose running `/evaluate-issue-plan` for those issues.
     - If any `plan-pending` issues HAVE a plan evaluation AND user feedback comments (from rjskene) posted AFTER the evaluation → propose re-running `/plan-issue N` to revise.
   ...
   - Else if any issues have no pipeline label and are not blocked and are not labeled `human`:
     - **Before proposing planning:** verify every ready issue has a fresh `## Classification` comment (the cache check from step 2 considers a comment fresh when its `createdAt > issue.updatedAt`). If any ready issue lacks a fresh classification, propose running `/classify-issue N` for those issues first. Do NOT advance to planning until all ready issues are classified — classify-issue writes both the comment and the path label together.
...
7. **On confirmation:**
   ...
   **For plan evaluation (plan-pending → plan-reviewed):** Run `/evaluate-issue-plan N` for each issue needing evaluation. If multiple issues need evaluation, spawn one Agent per issue in parallel (foreground), each invoking `/evaluate-issue-plan N`. The evaluate-issue-plan skill is read-only — it reads the plan comment and codebase, posts an evaluation comment, and updates labels. If the verdict is "Approve," the label changes to `plan-reviewed`. If "Revise," the label stays `plan-pending` and the evaluation comment explains what needs to change.

   **For planning (no label → plan-pending) or re-planning (plan-pending with user feedback):** Run `/plan-issue N` for each issue. If multiple issues need planning, spawn one Agent per issue in parallel (foreground), each invoking `/plan-issue N`. If multiple issues share a branch (discovered from issue body/comments or matching branch names), plan them together in a single agent call. The plan-issue skill reads prior comments (including user feedback) and produces a revised plan when feedback exists.
   ...
   for N in <planned_issues>; do
     ...
     if [ "$PLAN_COUNT" -eq 0 ]; then
       echo "WARNING: Issue #$N has no plan comment. Re-running /plan-issue $N."
   ...
   **For execution (plan-approved → worktree setup):** For each approved issue's branch (deduplicated — issues sharing a branch get one worktree):
   1. Run the setup script with the issue number:
      ...
      ```
      This creates the worktree at `.claude/worktrees/wt-<issue_number>-<slug>`, copies `.claude/settings.local.json`, installs dependencies, and seeds the dev database.

   2. After ALL worktrees are set up, print a summary with the frontend URL for each issue. The frontend port is `4000 + issue_number`:
      ```
      WORKTREES READY
      ================================================================
      Issue  Frontend URL                        Worktree Path
      ----------------------------------------------------------------
      #N     http://<droplet-ip>:<4000+N>        .claude/worktrees/wt-N-<slug>
      ================================================================
      ```
   ...
   3. Ask: "Launch mode? (terminal / tmux / remote-control / manual) | Skip permissions? (y/n)"
      ...
      - **terminal** (default) — new Terminal.app window per issue with interactive Claude, auto-fires `/execute-issue-plan N`. User monitors and interacts locally.
      - **tmux** — tmux windows in a `dev` session, auto-fires `/execute-issue-plan N`
   ...
   4. **If user says tmux and there are 2+ issues** — use the queue runner to manage concurrency (max 3 at a time):
      ...
      ```bash
      tmux send-keys -t dev "bash .claude/scripts/run-queue.sh [--skip-permissions] <issue1> <issue2> ..." Enter
      ```
   ...
   6. **If user says manual** — print manual instructions:
      ...
      Interactive: cd <worktree-path> && claude [--dangerously-skip-permissions] "/execute-issue-plan N"
...
8. **Merge orchestration** — after all evaluations complete and verdicts are "Approved", the pipeline handles merging:
   ...
   **Only run this step when the user confirms they want to merge.** After evaluation agents finish, report the verdicts and ask: "Merge approved PRs? (yes / no)"
   ...
   for PR_NUM in $(gh pr list --repo HTS-COLLAB-ORG/claude-pipeline --state open --json number --jq '.[].number'); do
     PR_TITLE=$(gh pr view $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --json title --jq '.title')
   ...
   1. For each approved PR, detect its base branch:
      ```bash
      gh pr view $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --json baseRefName --jq '.baseRefName'
      ```
   1b. Verify and retarget if needed — compare the PR's current base with the expected base from the worktree metadata:
      ```bash
      EXPECTED_BASE=$(cat <worktree-path>/.claude/base-branch 2>/dev/null || echo "staging")
      ACTUAL_BASE=$(gh pr view $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --json baseRefName --jq '.baseRefName')
   ...
   2. Check for merge conflicts:
      ```bash
      gh pr view $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --json mergeable --jq '.mergeable'
      ```
   3. If conflicts exist, attempt rebase in the worktree:
      ```bash
      cd <worktree-path>
      BASE_BRANCH=$(cat .claude/base-branch 2>/dev/null || echo "staging")
   ...
   4. Merge PRs sequentially to avoid cascading conflicts. Before each merge, validate the PR title against the Conventional Commits format — release-please reads the squash commit on merge, so a non-conforming title breaks automated versioning and CHANGELOG generation.
      ...
      ```bash
      gh pr merge $PR_NUM --repo HTS-COLLAB-ORG/claude-pipeline --squash --delete-branch
      gh issue edit <N> --repo HTS-COLLAB-ORG/claude-pipeline --add-label "merged" --remove-label "pr-open"
      gh issue close <N> --repo HTS-COLLAB-ORG/claude-pipeline
      ```
