...
description: Independently evaluate an implementation plan posted on a GitHub issue. Posts findings and adds plan-reviewed label. Usage: /evaluate-issue-plan <issue_number>
...
4. **Check for conflicts with in-flight work:**
   ```bash
   gh pr list --repo HTS-COLLAB-ORG/claude-pipeline --state open --json number,title,files \
...
5. **Post evaluation comment on the issue:**
   ```bash
   gh issue comment <N> --repo HTS-COLLAB-ORG/claude-pipeline --body "<evaluation>"
   ```
...
6. **Update labels:**
   ...
   ```bash
   gh issue edit <N> --repo HTS-COLLAB-ORG/claude-pipeline --add-label "plan-reviewed" --remove-label "plan-pending"
   ```
