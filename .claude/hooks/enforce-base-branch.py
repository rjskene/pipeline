...
hook falls back to the project default ``staging``
substituted by install.sh. This lets a worktree cut from ``next`` (or
any other non-default base) correctly gate its own ``gh pr create``
calls without re-running install.sh.
