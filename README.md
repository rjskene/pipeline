...
- **Dynamic model routing** — the execute model is resolved per path (`PIPELINE_PATH_{B,D}_MODEL_EXECUTE`, with a `low-blast` eligibility predicate); PATH D defaults to Sonnet, PATH B to Opus since #1420. The Opus pr-eval backstop is mandatory and never tier-dropped. See [docs/cost-architecture.md](docs/cost-architecture.md) and [docs/analysis/model-downsampling.md](docs/analysis/model-downsampling.md).
...
- **PATH B execute** — one execute agent applies the `tdd-implementer` discipline inline (red→green→commit per plan task); `evaluate-issue-pr` is the sole review gate.
