...
- **Auto-pause** — near the plan limit the run self-pauses (`pause-5h`) or hard-halts (`halt-7d`) instead of burning into a rate-limit wall (7d wins over 5h). See [docs/usage-gate.md#auto-pause-pause-5h--halt-7d](docs/usage-gate.md#auto-pause-pause-5h--halt-7d).
- **Auto-refire** — a recurring re-check cron auto-resumes a paused run once the 5h window resets, so long campaigns ride through the budget reset unattended. See [docs/usage-gate.md#auto-refire-recurring-re-check-resume](docs/usage-gate.md#auto-refire-recurring-re-check-resume).
...
- **Tokenomics** — `/pipeline:tokenomics` renders per-bucket / per-stage / per-structure cost and latency with the B→D breakeven over the gated `agent-costs.jsonl` log (dogfood-only). See [docs/observability.md#agent-cost-capture--pipelinetokenomics](docs/observability.md#agent-cost-capture--pipelinetokenomics) and [docs/tokenomics/README.md](docs/tokenomics/README.md).
