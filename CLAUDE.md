...
**Consumer-required rendered scripts.** Plugin skills (`skills/run`, `skills/fullsend`, etc.) invoke worktree/dispatch helpers as `bash .claude/scripts/<name>.sh` — referencing the **consumer's** `.claude/scripts/` path, NOT `${CLAUDE_PLUGIN_ROOT}/scripts/`. The plugin ships these as `scripts/<name>.sh.template` files; the consumer's working tree carries the rendered `.sh` either from (a) a prior subtree install preserved by `scripts/migrate-from-subtree.sh`, or (b) this repo's checked-in `.claude/scripts/` for dogfood sessions. These rendered scripts are load-bearing: deleting them breaks `/pipeline:run`. The doctor's `skill_files_residual` check classifies them as `consumer-required` (separate from `consumer-owned`) and `--fix residual` will not propose deletion. The architectural follow-up (render at install time, or rewrite plugin skills to invoke `${CLAUDE_PLUGIN_ROOT}/scripts/` directly) is tracked in **#215**; once it lands, the `consumer-required` `.template`-branch in `doctor.sh` becomes deletable.

**Dogfood drift gate (#252).** Until #215 ships, the dogfood `.claude/scripts/*.sh`, `.claude/hooks/{*.py,*.sh}`, and `.claude/agents/*.md` must be byte-identical to their plugin counterparts at `scripts/*.sh`, `hooks/*`, and `agents/*.md`. The CI gate `tests/test-dogfood-consumer-scripts-in-sync.sh` runs on every push/PR and fails if any pair drifts (or if a consumer copy exists without a plugin counterpart — reverse drift). Any PR that modifies `scripts/<name>.sh` MUST also `cp` to `.claude/scripts/<name>.sh` in the same commit; humans forget, the gate enforces. Relatedly, the doctor's `consumer_drift` check resolves the plugin counterpart against the **per-project active plugin cache** (read from `~/.claude/plugins/installed_plugins.json` via `PIPELINE_RESOLVE_MODE=active-project`), not the highest-version stable-marketplace entry — so when a project pins a `pipeline@claude-pipeline-dev` RC install, drift is compared against that RC, not against stable. The marketplace-name fallback remains the default for backward compatibility; only `consumer_drift` opts into the active-project mode via `PIPELINE_DIFF_PLUGIN_ROOT_MODE=active-project`. The `record consumer_drift` detail line surfaces the resolved active root (`... (against <path>)`) so findings self-document.
