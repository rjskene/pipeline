...
**For execution (plan-approved → worktree setup).** After the worktree is set up, read each approved issue's labels and route by tier:
   ...
   - **PATH C** (`multi-task`): dispatch inline by DEFAULT — the orchestrator reads the `## Implementation Plan` and fans out one `Agent(subagent_type='tdd-implementer', description='execute-issue-plan #<N> target=<dir>/ ...', prompt: 'target=<dir>/ ...')` per `target=<dir>`, bounded by the **conservative inline-C concurrency cap of 1–2 concurrent `tdd-implementer`** (distinct from the A/B/D max-3 foreground bound; justified by the bigger-than-B orchestrator-context cost — plan-read + N-target sequencing + PR creation + N inline dispatch/return contexts held in-orchestrator; tune upward only after the live branch test). Then the orchestrator handles push + `gh pr create` + labels itself. The orchestrator MUST NOT `Edit`/`Write` impl files directly — the `enforce-path-c-delegation` hook blocks it and authorizes only files under a dispatched `target=<dir>` sentinel. `tdd-implementer` is dispatched from the top level as a hard leaf executor (no grandchild dispatch). **Under `--spawn`, revert to the legacy `spawn-claude.sh` / `run-queue.sh` → `tdd-implementer` fan-out** (the reversible #750 escape hatch). **Live branch-test merge gate:** inline-C's first rollout is operator-gated — run ONE real PATH C issue through inline fan-out FROM THE FEATURE BRANCH, confirm the main-session context load is tolerable, and dispatch with `--manual-merge` (NO auto-merge) until validated.
