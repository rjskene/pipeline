...
**For execution (plan-approved → worktree setup).** After the worktree is set up, read each approved issue's labels and route by tier:
   ...
   - **Per-path execute MODEL routing (#1042 — Sonnet is the DEFAULT, opt-OUT).** Before dispatching a PATH B or PATH D **execute** `Agent`, source `pipeline.config` and read `PIPELINE_PATH_B_MODEL_EXECUTE` (PATH B) / `PIPELINE_PATH_D_MODEL_EXECUTE` (PATH D). The value is the `model` enum: `sonnet`|`opus`|`haiku`. **When the var is unset or empty, the effective value defaults to `sonnet`** (#1042 flipped the polarity: the old default was unset → no `model=` → inherit Opus; Sonnet-on-execute is now the shipped default and an operator must opt OUT). Pass `model=<effective-value>` to that `Agent(...)` dispatch — the resolved value, defaulting to `sonnet` when unset. An operator **opts OUT** by setting the var explicitly to `=opus` (or any non-sonnet enum value); that explicit value is **honored verbatim** — only the unset/empty case defaults to `sonnet`, never an explicit assignment. (Setting `=opus` reproduces the old inherit-Opus behavior for that path.) Note: this `sonnet` default is still subject to the carve-outs below — a W2 high-uncertainty PATH B issue forces Opus, and a `needs-browser` PATH D issue forces Opus — and **pr-eval is NEVER defaulted to Sonnet** (the W3 backstop, restated at the end of this block).
     ...
     **Phase 2 split-role dispatch (#881 — `PIPELINE_PATH_B_SPLIT_ROLE`).** When `PIPELINE_PATH_B_SPLIT_ROLE=true` (default `false`), PATH B execute becomes **TWO sequential agents in the existing worktree** (not two worktrees):
     ...
     - **(ii) Implementer** — model from `PIPELINE_PATH_B_MODEL_EXECUTE` (unset → `opus` = same-model compliance-only split-role); a **W2 carve-out forces `opus`**. Greens the suite **test-by-test, additive-only** (may ADD tests, never modify/delete the locked `[split-role-red]` tests). Runs **after** the test-author commit lands.
