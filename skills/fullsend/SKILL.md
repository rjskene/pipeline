...
**For execution (plan-approved → worktree setup).** After the worktree is set up, read each approved issue's labels and route by tier:
   ...
   - **Per-path execute MODEL routing (#1042 — Sonnet is the DEFAULT, opt-OUT).** Before dispatching a PATH B or PATH D **execute** `Agent`, source `pipeline.config` and read `PIPELINE_PATH_B_MODEL_EXECUTE` (PATH B) / `PIPELINE_PATH_D_MODEL_EXECUTE` (PATH D). The value is the `model` enum: `sonnet`|`opus`|`haiku`. **When the var is unset or empty, the effective value defaults to `sonnet`** (#1042 flipped the polarity: the old default was unset → no `model=` → inherit Opus; Sonnet-on-execute is now the shipped default and an operator must opt OUT). Pass `model=<effective-value>` to that `Agent(...)` dispatch — the resolved value, defaulting to `sonnet` when unset. An operator **opts OUT** by setting the var explicitly to `=opus` (or any non-sonnet enum value); that explicit value is **honored verbatim** — only the unset/empty case defaults to `sonnet`, never an explicit assignment. (Setting `=opus` reproduces the old inherit-Opus behavior for that path.) Note: this `sonnet` default is still subject to the carve-outs below — a W2 high-uncertainty PATH B issue forces Opus, and a `needs-browser` PATH D issue forces Opus — and **pr-eval is NEVER defaulted to Sonnet** (the W3 backstop, restated at the end of this block).
     ...
     **Phase 1 scope-widen (#881 — `PIPELINE_PATH_B_ELIGIBLE_SCOPE`; #1042 default `all`).** Source `pipeline.config` and read `PIPELINE_PATH_B_ELIGIBLE_SCOPE`. **When `PIPELINE_PATH_B_ELIGIBLE_SCOPE` is unset or empty, it defaults to `all`** (#1042 flipped the polarity: the old default was `low-blast`; the scope now defaults to `all` so the Sonnet-on-execute default applies to every eligible PATH B issue, not just the low-blast lane). An operator **opts OUT** by setting `=low-blast`, which restores the old low-blast-only restriction (the line-445 eligibility gate then governs — Sonnet only on a `low-blast` verdict). When the resolved scope is `all` AND the issue is PATH B **WITHOUT a W2 carve-out** — the high-uncertainty regex that `path-b-execute-eligible.sh` applies (now the **word-bound** shared pattern in `scripts/_high-uncertainty-match.sh`: vocabulary `concurrency` / `auth` / `lock` / `race` / `deadlock` / `security` / `crypto` / `migration` / `data-loss`, with `auth`/`lock`/`race` word-bound so `authoring`/`block`/`trace` do NOT trip it — #1039; reuse that script's regex — do NOT redefine it here and do NOT raw-substring-match) OR the `needs-browser` label — pin `model=sonnet` even on a **high-blast** eligibility verdict (the widen overrides the low-blast restriction above). A W2 carve-out issue still **inherits Opus** (never widened, never downshifted) — under the default `all` scope this carve-out is now the **sole safety boundary** keeping genuinely risky PATH B work (security / auth / concurrency / migration / data-loss) on Opus, so it depends on #1039's word-bound regex correctness (a false-negative would silently route risky work to Sonnet by default). `path-b-execute-eligible.sh` is **UNCHANGED** in contract: the `ELIGIBLE=<low-blast|high-blast>` line is still emitted and relayed in the run log as an **advisory** audit of what the gate WOULD have done.

     **Phase 2 split-role dispatch (#881 — `PIPELINE_PATH_B_SPLIT_ROLE`).** When `PIPELINE_PATH_B_SPLIT_ROLE=true` (default `false`), PATH B execute becomes **TWO sequential agents in the existing worktree** (not two worktrees):
     - **(i) Opus test-author (`model=opus` ALWAYS, regardless of any knob).** Authors the full failing suite per the approved plan, verifies each test is **red-for-the-right-reason**, then makes a **single commit** with the literal `[split-role-red]` substring in the **commit subject**. Pre-existing-test updates (golden refresh, changed contract) belong **IN this commit**.
     - **(ii) Implementer** — model from `PIPELINE_PATH_B_MODEL_EXECUTE` resolved per the #1042 read-site rule above (unset → defaults to `sonnet`; an explicit `=opus` opt-out is honored verbatim and yields the same-model compliance-only split-role); a **W2 carve-out forces `opus`** regardless. Greens the suite **test-by-test, additive-only** (may ADD tests, never modify/delete the locked `[split-role-red]` tests). Runs **after** the test-author commit lands.

     **PATH D stays unconditional EXCEPT needs-browser** — the resolved `PIPELINE_PATH_D_MODEL_EXECUTE` (defaulting to `sonnet` when unset, per #1042 above) applies to **all** PATH D issues with no eligibility predicate (all D is in-lane by construction), with ONE carve-out: when a PATH D issue carries the `needs-browser` label, pass **NO** `model=` param — it inherits the orchestrator's Opus (issue #960: the #950 Sonnet-on-execute pilot validated shell-helper fixtures only; browser/UI execute was never measured, so a needs-browser D must not downshift). Example (PATH D): `Agent(subagent_type='tdd-implementer', model='sonnet', description='execute-issue-plan #<N> ...', prompt='cd <wt>; ...')` when the resolved `PIPELINE_PATH_D_MODEL_EXECUTE=sonnet` (the default), else — when an operator has opted OUT with `=opus` — the corresponding `model=opus` form. **This gate applies to the execute dispatch ONLY: the pr-eval dispatch is NEVER gated and is NEVER defaulted to Sonnet** — the independent evaluator stays on the inherited Opus backstop (dropping the evaluator's tier would remove the regression catcher that makes a cheaper-execute default safe; this is the W3 invariant). The knobs are host-overridable (gitignored `pipeline.config`); the shipped default is now Sonnet-on-execute (#1042) — a fresh install runs Sonnet on eligible PATH B + all PATH D execute, and an operator restores the conservative Opus tier by setting `=opus` / `=low-blast` (or commenting the knobs).
