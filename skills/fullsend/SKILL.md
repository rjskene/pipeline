...
- **Hard block (triggers a scoped halt):** `block-mergestate`, `block-mergeable`, `block-verdict`, `block-capability-refused`, `block-base-mismatch`, or a CI failure whose retry budget is exhausted. When such a block leaves a wave-N issue's PR unmerged, compute its **dependency closure** and halt only that closure.
