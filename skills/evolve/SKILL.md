...
- `resume` — invoking `resume` IS the un-pause, so it FIRST clears the kill switch with `gh issue edit "$TRACKER" --repo "$PIPELINE_REPO" --remove-label paused`, then runs Step 0 in full, then continues at the recorded step (`done` → next cycle). Otherwise Step 0's `paused` check STOPs every resume.
