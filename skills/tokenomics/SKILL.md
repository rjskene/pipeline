...
- **Role split** — cost breakdown across the execute roles the window contains: `single` for every execute since #1420, plus the historical `red` (Opus test-author) and `green` (implementer) rows captured while the split-role lane existed and still priced. Sourced from the `role` field (schema-v1 additive field, #1098); records with no `role` field bucket as `single` (legacy/fixture compat). Only present under `--tokenomics`; does not appear in the default report. When every record carries `role=single` — the steady state after #1420 — only a `single` row renders.
...
3. **Every table verbatim** in its own fenced code block, each preceded by a `**Table name**` + em-dash one-line READ — the takeaway (e.g., "cache dominates: 91% of tokens"), never a prose restatement of the rows. Order: bucket → per-stage cost → structure → role split → stage×structure → per-PATH → per-stage medians → top consumers/slowest (may share one block) → B→D breakeven → coverage-health → trend per-day → trend per-PR → task-latency.
