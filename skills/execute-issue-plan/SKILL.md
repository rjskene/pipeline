...
5. **Implement the approved plan.** Follow the plan's `**Tasks (ordered):**` section exactly — it carries the path-specific Task 0 directive (PATH A: flat edits; PATH B: invoke `superpowers:test-driven-development`; PATH C: dispatch `tdd-implementer` subagents with `target=<dir>` sentinels).
   ...
   **Harness-neutral dispatch (#983).** Only the per-leaf dispatch CALL is harness-specific: `scripts/dispatch-leaf.sh` (keyed on `PIPELINE_HARNESS`) branches the verb between the Claude Code in-session `Agent` tool and the Codex `spawn_agent`/`wait_agent`/`close_agent` agent-control surface. The `path-c-split-worktree.sh` setup → reassemble → teardown lifecycle above is harness-INVARIANT — invoked verbatim on both harnesses, with no change to the PATH C delegation semantics (`enforce-path-c-delegation` sentinel rules, per-target commit isolation, cherry-pick reassembly).
