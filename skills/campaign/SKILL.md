...
- **Resolver-driven execute dispatch (#1056):** each leg's PATH B / PATH D execute dispatch routes through `scripts/resolve-execute-dispatch.sh` (the single-source MODEL + split-role + carve-out resolver) exactly as fullsend's Step 6 does — inherited by reference, **no logic duplicated here**. Edit the resolver / fullsend Step 6, and campaign legs pick it up.
