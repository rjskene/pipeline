...
6. **On confirmation:**
   ...
   **Dispatch routing by path tier.** After the worktree is set up (step 1 below), read each approved issue's labels:
   ...
   1. Run the setup script with BOTH positional args — `<branch-name>` AND `<issue-number>`. `<branch-name>` MUST be `feature/<slug>` where `<slug>` is derived from the issue title per the "Branch and worktree naming convention" block above; `<issue-number>` is the bare integer:
      ```bash
