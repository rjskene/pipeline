...
- `container_skills_validity` — Pass when `PIPELINE_CONTAINER_SKILLS` is unset (default `evaluate-issue-pr`), empty (explicit disable), or contains only canonical skill names (derived at runtime from `${CLAUDE_PLUGIN_ROOT}/skills/`). Warn when an unknown skill name appears — points at a typo in `pipeline.config` or a skill name that no longer exists. Read-only; never fails. See #321.
