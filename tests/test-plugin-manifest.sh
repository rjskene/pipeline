...
# NO hard 'dependencies' key — the pipeline depends on no other plugin at all:
# #1419 inlined the last of the procedures the skills used to delegate to, so
# there is not even a soft dependency left to document. A hard dep here would
# also break installs that resolve a same-named plugin from a different
# marketplace, which is why the field stayed empty before #1419 too.
