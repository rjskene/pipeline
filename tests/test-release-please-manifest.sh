...
assert "manifest \".\" version matches plugin.json version" "python3 -c 'import json,sys,re; m=json.load(open(\"$MAN\")); p=json.load(open(\"$REPO_ROOT/.claude-plugin/plugin.json\")); v=m[\".\"]; sys.exit(0 if v==p[\"version\"] and re.match(r\"^\\d+\\.\\d+\\.\\d+(-[A-Za-z0-9.-]+)?\$\", v) else 1)' 2>/dev/null"
