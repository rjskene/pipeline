...
HITS=$(grep -rEn '/plan-issue|/classify-issue|/execute-issue-plan|/evaluate-issue-plan|/evaluate-issue-pr|/create-issues|/worktree-sync|/pipeline\b' \
  ...
  | grep -vE 'pipeline:plan-issue|pipeline:classify-issue|pipeline:execute-issue-plan|pipeline:evaluate-issue-plan|pipeline:evaluate-issue-pr|pipeline:create-issues|pipeline:worktree-sync|pipeline:run|pipeline:\$\{SKILL\}|pipeline\.config|pipeline-cleanup|/tmp/pipeline|/pipeline/SKILL|skills/pipeline|/proc/sys' \
  || true)
if [ -n "$HITS" ]; then
