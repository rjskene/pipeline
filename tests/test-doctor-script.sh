...
# ---------------------------------------------------------------------------
# Case 10: dev_marketplace_on_main
# ---------------------------------------------------------------------------
# Helper: write the known_marketplaces.json fixture and echo its path.
mk_known_marketplaces() {
  local path="$1" payload="$2"
  mkdir -p "$(dirname "$path")"
  printf '%s' "$payload" > "$path"
}

echo "Case 10a: dev marketplace not registered"
FX=$(fresh_fx fx-devmkt-absent)
KM="$TMP/km-absent.json"
mk_known_marketplaces "$KM" '{"claude-pipeline":{"source":{"path":"/some/path"}}}'
run_helper "$FX" LABELS_JSON="$ALL_LABELS_JSON" CLAUDE_PLUGIN_LIST="claude-pipeline 0.4.0" DOCTOR_KNOWN_MARKETPLACES_FILE="$KM"
out="$(cat "$FX/out")"; rc="$(cat "$FX/rc")"
grep -qE '^CHECK: dev_marketplace_on_main status=pass detail=dev marketplace not registered' <<<"$out" \
  && pass_msg "devmkt-absent: pass detail" \
  || { fail_msg "devmkt-absent: wrong detail"; echo "$out" | sed 's/^/    /'; }
[ "$rc" = "0" ] && pass_msg "devmkt-absent: exit 0" || fail_msg "devmkt-absent: exit $rc"

echo "Case 10b: dev marketplace registered, clone on main"
FX=$(fresh_fx fx-devmkt-main)
DEV_CLONE="$TMP/dev-clone-main"
rm -rf "$DEV_CLONE"
mkdir -p "$DEV_CLONE/.claude-plugin"
(
  cd "$DEV_CLONE"
  git init -q
  git config user.email t@t
...
touch "$DEV_CLONE/.claude-plugin/marketplace-dev.json"
KM="$TMP/km-main.json"
mk_known_marketplaces "$KM" "{\"claude-pipeline-dev\":{\"source\":{\"path\":\"$DEV_CLONE/.claude-plugin/marketplace-dev.json\"}}}"
run_helper "$FX" LABELS_JSON="$ALL_LABELS_JSON" CLAUDE_PLUGIN_LIST="claude-pipeline 0.4.0" DOCTOR_KNOWN_MARKETPLACES_FILE="$KM"
out="$(cat "$FX/out")"; rc="$(cat "$FX/rc")"
grep -qE '^CHECK: dev_marketplace_on_main status=pass detail=clone on main' <<<"$out" \
  && pass_msg "devmkt-main: pass detail=clone on main" \
  || { fail_msg "devmkt-main: wrong detail"; echo "$out" | sed 's/^/    /'; }
[ "$rc" = "0" ] && pass_msg "devmkt-main: exit 0" || fail_msg "devmkt-main: exit $rc"

echo "Case 10c: dev marketplace registered, clone on non-main branch"
FX=$(fresh_fx fx-devmkt-staging)
DEV_CLONE="$TMP/dev-clone-staging"
rm -rf "$DEV_CLONE"
mkdir -p "$DEV_CLONE/.claude-plugin"
(
  cd "$DEV_CLONE"
  ...
  git config user.name t
  git commit --allow-empty -q -m init
  git checkout -q -b staging 2>/dev/null || git branch -q -M staging
) >/dev/null 2>&1
touch "$DEV_CLONE/.claude-plugin/marketplace-dev.json"
KM="$TMP/km-staging.json"
mk_known_marketplaces "$KM" "{\"claude-pipeline-dev\":{\"source\":{\"path\":\"$DEV_CLONE/.claude-plugin/marketplace-dev.json\"}}}"
run_helper "$FX" LABELS_JSON="$ALL_LABELS_JSON" CLAUDE_PLUGIN_LIST="claude-pipeline 0.4.0" DOCTOR_KNOWN_MARKETPLACES_FILE="$KM"
out="$(cat "$FX/out")"; rc="$(cat "$FX/rc")"
grep -qE '^CHECK: dev_marketplace_on_main status=warn detail=dev marketplace clone is on staging, not main — installed version may lag main' <<<"$out" \
  && pass_msg "devmkt-staging: warn detail" \
  || { fail_msg "devmkt-staging: wrong detail"; echo "$out" | sed 's/^/    /'; }
[ "$rc" = "0" ] && pass_msg "devmkt-staging: warn does not fail exit" || fail_msg "devmkt-staging: exit $rc"
