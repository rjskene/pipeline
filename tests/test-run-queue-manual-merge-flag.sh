#!/bin/bash
# Lint the canonical plugin-source templates for --manual-merge wiring.
# Does NOT touch .claude/scripts/*.sh — those are render outputs and are
# regenerated by the harness.
set -u

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
RQ="${ROOT}/scripts/run-queue.sh.template"
SC="${ROOT}/scripts/spawn-claude.sh.template"
FAILED=0
