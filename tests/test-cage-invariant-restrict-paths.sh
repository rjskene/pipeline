#!/bin/bash
# Cage invariant (#1304) — hooks/restrict_paths.py must DENY a Write that
# targets a path outside the project boundary.
#
# This is a characterization test for a guardrail the harness relies on. It
# pins the DENY CONTRACT, not an exit code: Claude Code treats a PreToolUse
# hook as blocking when it exits 2 OR when stdout carries
# `hookSpecificOutput.permissionDecision == "deny"`. Either satisfies the
# invariant; nothing here pins rc == 1.
#
# Harness shape copied from tests/test-restrict-paths-hook.sh (env-isolated
# subprocess, PASS/FAIL counters, mktemp -d fixture + trap, non-zero exit only
# on FAIL). All fixture state lives under the mktemp dir.
set -euo pipefail
...
HOOK_NAME="restrict_paths.py"
HOOK="$REPO_ROOT/hooks/$HOOK_NAME"
# hooks/restrict_paths.py is contract-compliant today (it exits 2), so the
...
GUARDED='{"tool_name":"Write","tool_input":{"file_path":"/etc/cage-invariant-probe.txt"}}'
CONTROL=$(printf '{"tool_name":"Write","tool_input":{"file_path":"%s/scratch.txt"}}' "$PROJ")

echo "Cage invariant: $HOOK_NAME"

assert_denied "Write to /etc outside the project boundary must be DENIED" "$GUARDED"
assert_allowed "Write inside the project boundary must be ALLOWED" "$CONTROL"
