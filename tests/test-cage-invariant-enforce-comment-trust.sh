#!/bin/bash
# Cage invariant (#1304) — hooks/enforce-comment-trust.py must DENY a raw
# issue-comment read that bypasses the trusted comment filter.
#
...
#
# The hook is registered in the dogfood consumer settings rather than in the
# published plugin manifest; this test drives the file directly, so where it
# is registered is irrelevant to the invariant.
#
...
HOOK_NAME="enforce-comment-trust.py"
...
# The untrusted --json field name is assembled at runtime so this file never
# contains the raw bypass command verbatim — otherwise the file is itself a
# scan false-positive and the live hook blocks the Bash calls that author it.
# (Precedent: the SENT assembly at
# tests/test-auto-merge-gate-capability-refusal.sh:42.)
FIELD="comm""ents"

GUARDED=$(bash_payload "gh issue view 42 --json body,${FIELD}")
CONTROL=$(bash_payload "scripts/filter-trusted-comments.sh 42")

echo "Cage invariant: $HOOK_NAME"

assert_denied "raw issue-comment read bypassing the trust filter must be DENIED" "$GUARDED"
assert_allowed "read routed through the trusted helper must be ALLOWED" "$CONTROL"
