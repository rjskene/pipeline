...
When `PIPELINE_HARNESS=codex` (resolved via `scripts/platform.sh`), doctor additionally emits a block of Codex-mode checks — `codex_installed`, `codex_multi_agent_enabled`, `codex_hooks_wired` (FAIL grade) plus `codex_hooks_trusted`, `codex_mcp_reachable` (WARN grade) — validating the operator's `~/.codex/` install via static config reads only. The block is fully gated: Claude Code runs emit zero `codex_*` lines. See [docs/codex-dogfood-setup.md](codex-dogfood-setup.md).
