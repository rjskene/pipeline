...
**CAPABILITY-REFUSED: sentinel detector (#1233).** `scripts/check-capability-refusal.sh` is the mechanical listener for the `CAPABILITY-REFUSED:` contract (`agents/tdd-implementer.md`, #1225). It scans `.result` only (never `.prompt`, which can legitimately quote the contract) and resolves exactly one of five coverage states per issue: `leaf-refused` (a real refusal — blocks auto-merge via `block-capability-refused`), `no-refusal` (the only confident clean — at least one in-scope file yielded leaf text and none carried the sentinel), `async-dispatch` (#1361 — every opened record is a `status:"async_launched"` background-dispatch stub with an empty `.result`; the leaf's result never reached the hook, so nothing here could ever carry leaf text — a structural skip, not evidence of anything), `no-leaf-output` (in-scope files were opened, at least one is NOT an async stub, yet none yielded leaf text — proves nothing; this is the state of every record before the result-capture fix above), and `no-sources` (nothing scannable resolved for the issue). Only `no-refusal` is a confident clean; `no-leaf-output` / `no-sources` are surfaced with a gate-level `WARN` rather than silently treated as green, and `async-dispatch` with a one-line `NOTE`. The stdout line carries `SCANNED=` / `WITH_OUTPUT=` / `ASYNC=` counters.
...
Gated on the same [`PIPELINE_LOGS_ENABLED`](#pipeline_logs_enabled-gate) flag as `tool-use.log` / `agent-costs.jsonl` (disabled — no file, no directory touched — until a host opts in), evaluated against the original `project_dir`; the log-dir resolves worktree-aware via `git -C <project_dir> rev-parse --git-common-dir` (same mechanism as `check-capability-refusal.sh --resolve-sources`), whose parent is the MAIN checkout root for both the checkout and any linked worktree, falling back to `project_dir` on failure — so denials from linked worktrees land in the one durable main-checkout file. The helper is **fail-open by construction**: its entire body is wrapped in one `try/except Exception: pass`, so a logging failure can never turn a deny into a crash or an allow, and it writes no error log. No guard's decision logic, exit code, or stderr text changes — this is a pure audit-trail addition.
