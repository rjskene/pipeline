...
2. When ready to release, open the promotion PR `staging` → `main`: `gh pr create --repo <owner/repo> --base main --head staging --title "chore: promote staging to main" --body "…"` — and merge-commit it (`gh pr merge --merge`) — fast-forward is acceptable when branches are linear, but merge-commits are the canonical path because they preserve per-PR conventional-commit history for release-please. The `enforce-base-branch.py` hook admits this one shape only when `--base` equals `PIPELINE_RELEASE_BRANCH` (default `main`) AND `--head` equals the configured `PIPELINE_BASE_BRANCH` (#1356); any other head, or an omitted `--head`, is still denied.
