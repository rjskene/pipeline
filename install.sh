...
# Apply default for PIPELINE_TMUX_SESSION if pipeline.config predates this var.
# Falling back to "dev" preserves legacy behavior; envsubst would otherwise
# substitute an empty string into SKILL.md.
PIPELINE_TMUX_SESSION="${PIPELINE_TMUX_SESSION:-dev}"
...
ENVSUBST_VARS+=' $PIPELINE_TMUX_SESSION'
